                ...
                Router.AssesmentWithID(Router.AID)//.Child("endDate")
                  .GetValueAsync().ContinueWith(task =>
                  {
                      ...
                      else if (task.IsCompleted)
                      {
                          ...
                          //    for (int i = 0; i < 7; i++)
                          //    {
                          //        Router.DataWithAssID(Router.AID).Child(str).Child(j.ToString()).Child(((char)(i + 65)).ToString()).UpdateChildrenAsync(dic2);
                          //    }
                          //}
                          ...
                          foreach(double[] dat in dummyData) //(int i = 0; i < dummyData.Length; i++)
                          {
                              ...
                              //Debug.Log("Pushing Now!!");
                              Router.DataWithAssID(Router.AID).Child(str).Child((i).ToString()).Child(((char)(65)).ToString()).SetValueAsync(dic2); //UpdateChildrenAsync
                              dic2.Clear();
                          }
